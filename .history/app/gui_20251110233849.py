...
class FLAC2AIFFApp:
    ...
    def _sanitize_filename(self, filename: str) -> str:
        """Sanitize filename using whitelist approach.
        
        Allowed characters:
        - Letters: A-Z, a-z (including accented characters like é, ä, ø)
        - Numbers: 0-9
        - Space
        - Safe punctuation: - _ ( ) [ ] { } , . ' + &
        
        Forbidden characters (replaced with space):
        - / \ : * ? " < > | % # and control characters
        """
        if not filename:
            return "Unknown"
        
        # Build whitelist: Unicode word chars (letters/numbers), space, and safe punctuation
        # Keep: letters, numbers, space, and: - _ ( ) [ ] { } , . ' + &
        sanitized = ""
        for char in filename:
            # Check if character is allowed
            if char.isalnum() or char.isspace():
                # Letters, numbers, and spaces are always allowed
                sanitized += char
            elif char in "-_()[]{},.'+&":
                # Safe punctuation
                sanitized += char
            else:
                # Forbidden character - replace with space
                sanitized += " "
