...
class FLAC2AIFFApp:
    ...
    def _set_app_icon(self) -> None:
        ...
        try:
            ...
            print(f"No icon file found. Checked: {[str(p) for p in icon_paths[:3]]}")
        except Exception as e:
    ...
    def _conversion_complete(self, converted: int, failed: int, total: int) -> None:
        ...
        if converted > 0:
            ...
            # Create messagebox with icon
            msg = tk.Toplevel(self.root)
            msg.withdraw()  # Hide initially
            if self.icon_image:
                msg.iconphoto(True, self.icon_image)
            messagebox.showinfo(
                "Conversion Complete",
            ...
            if self.icon_image:
                msg.iconphoto(True, self.icon_image)
            messagebox.showerror("Error", f"Failed to convert files. Check that ffmpeg is installed.", parent=msg)
            msg.destroy()
