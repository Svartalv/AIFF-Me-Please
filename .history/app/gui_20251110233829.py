...
class FLAC2AIFFApp:
    ...
    def _sanitize_filename(self, filename: str) -> str:
        ...
        Everything else is removed or replaced with space.
        """
        if not filename:
            return "Unknown"
        
        # Whitelist: letters, numbers, space, and safe punctuation
        # Allow Unicode letters (including accented) and numbers
        allowed_pattern = re.compile(r'[^\w\s\-_()\[\]{},.\'\+&]', re.UNICODE)
        
        # Replace forbidden characters with space
        sanitized = allowed_pattern.sub(' ', filename)
        ...
        for i, audio_path in enumerate(audio_files, 1):
            try:
                ...
                cmd = [
                    ...
                    "-ac", "2",                  # Stereo
                    "-c:a", "pcm_s24be",         # 24-bit PCM big-endian (AIFF format)
