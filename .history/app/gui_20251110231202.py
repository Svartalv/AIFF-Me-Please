...
class FLAC2AIFFApp:
            ...
            try:
                ...
                if count > 0:
                    self.status_label.config(
                        text=f"Found {count} FLAC file(s) in folder",
                        foreground="green"
                    )
                else:
                    self.status_label.config(
    ...
    def _find_ffmpeg(self) -> str:
        ...
        try:
            result = subprocess.run(["which", "ffmpeg"], capture_output=True, check=True)
    ...
    def _start_conversion(self) -> None:
        ...
        if not flac_files:
            messagebox.showwarning("No Files", "No FLAC files found in the input folder")
            return
        ...
        self.status_label.config(text=f"Converting {len(flac_files)} file(s)...", foreground="black")
        
        # Run conversion in thread
        thread = threading.Thread(
            target=self._convert_files,
            args=(flac_files, output_path, ffmpeg_path),
            daemon=True
        )
        thread.start()
    
    def _convert_files(self, flac_files: list, output_dir: Path, ffmpeg_path: str) -> None:
        ...
        for i, flac_path in enumerate(flac_files, 1):
            try:
                ...
                output_filename = flac_path.stem + ".aiff"
                ...
                while output_path.exists():
                    output_path = output_dir / f"{flac_path.stem} ({counter}).aiff"
                    counter += 1
                
                # Update status
                self.root.after(0, lambda f=flac_path.name, c=i, t=len(flac_files): 
                    self.status_label.config(
