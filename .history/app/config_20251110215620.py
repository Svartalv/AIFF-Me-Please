...
class Config:
    ...
    def __init__(self, config_path: str = None):
        ...
        if config_path is None:
            # Try to find config in app bundle or user's Application Support
            app_name = "FLAC2AIFF"
            if hasattr(self, '_get_app_support_path'):
                config_dir = self._get_app_support_path(app_name)
            else:
                # Fallback: use current directory or ~/Library/Application Support
                home = Path.home()
    ...
    def _get_app_support_path(self, app_name: str) -> Path:
        """Get Application Support path for macOS."""
        home = Path.home()
