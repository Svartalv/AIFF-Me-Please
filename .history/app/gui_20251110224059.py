...
class FLAC2AIFFApp:
    ...
    def _select_input_folder(self) -> None:
        ...
        selection = filedialog.askopenfilenames(
            title="Select FLAC files or folder",
            filetypes=[("FLAC files", "*.flac"), ("All files", "*.*")]
        )
        
        if selection:
            # If files are selected, use their parent directory
            if len(selection) == 1:
                path = Path(selection[0])
                if path.is_file():
                    # Single file selected - use its parent directory
                    folder = str(path.parent)
                    self.input_dir.set(folder)
                    # Auto-set output folder if empty
                    if not self.output_dir.get():
                        output_path = path.parent / f"{path.parent.name}_AIFF"
                        self.output_dir.set(str(output_path))
                else:
                    # It's a directory
                    self.input_dir.set(selection[0])
                    if not self.output_dir.get():
                        output_path = Path(selection[0]).parent / f"{Path(selection[0]).name}_AIFF"
                        self.output_dir.set(str(output_path))
            else:
                # Multiple files selected - use their common parent
                ...
                if len(paths) > 0:
                    ...
                    if all(p.parent == common_parent for p in paths):
                        ...
                        self.input_dir.set(folder)
                        if not self.output_dir.get():
                            output_path = common_parent / f"{common_parent.name}_AIFF"
                            self.output_dir.set(str(output_path))
                    else:
                        messagebox.showwarning(
                            "Multiple Directories",
                            "Selected files are in different directories. "
                            "Please select files from the same folder, or select a folder instead."
                        )
        else:
            # Fallback to folder selection if no files selected
