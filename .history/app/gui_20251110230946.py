...
class FLAC2AIFFApp:
    ...
    def _select_input_folder(self) -> None:
        ...
        if folder:
            ...
            try:
                # Quick check for FLAC files in the folder
                flac_files = [f for f in folder_path.iterdir() if f.is_file() and f.suffix.lower() == '.flac']
                ...
                if flac_count > 0:
                    ...
                    all_flac = list(folder_path.rglob("*.flac"))
                    if all_flac:
                        self.status_label.config(
                            text=f"Found {len(all_flac)} FLAC file(s) (including subfolders)",
                            foreground="green"
                        )
                    else:
                        self.status_label.config(
                ...
                # If we can't scan, just set the folder and let conversion handle it
                self.status_label.config(
